...
export class PostgreSQLManager {
    ...
    async initializeContainer(context: ExtensionContext) {
        ...
        await promiseExec(`docker run --network ${PostgreSQLManager.networkName} --name ${PostgreSQLManager.containerName} -e POSTGRES_USER=odoo -e POSTGRES_PASSWORD=odoo -e POSTGRES_DB=postgres -d postgres`);
    }
